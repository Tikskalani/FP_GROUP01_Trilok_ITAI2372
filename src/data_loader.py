...
def load_dataset(force_download: bool = False) -> pd.DataFrame:
    ...
    print(
        "[ExoNet] WARNING: could not load the live NASA KOI table — "
...
def _try_real_dataset(force_download: bool) -> pd.DataFrame | None:
    ...
    except Exception as exc:  # noqa: BLE001 — network/IO is genuinely unpredictable
        log.warning("Real KOI dataset unavailable: %s", exc)
        return None

    # Sanity check — if the columns we need are missing, treat as failure.
