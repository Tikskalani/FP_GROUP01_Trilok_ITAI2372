...
synthetic-fallback run, accuracy is closer to 0.85 — the loader will print a
warning at the top so you can tell which mode you're in.

Files written to `data/`:

- `kepler_koi.csv` — the cached NASA download
- `model.joblib` — the trained classifier
- `metrics.json` — the same numbers above as JSON
- `confusion_matrix.png` — the confusion matrix figure I use in the slides
